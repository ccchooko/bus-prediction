...
def haddle_missing(df):
    ...
    df_test = pd.read_csv('/Users/honey/Desktop/tianjinbus/toBePredicted_forUser.csv')
    df_train = df
    ...
    for terminalno in l_ter:
        ...
        if up_df_list:
            up_df = pd.concat(up_df_list).reset_index(drop=True)
            ter_up_df_list.append(up_df)
    ter_up_df = pd.concat(ter_up_df_list).reset_index(drop=True)
    print(ter_up_df.shape)
    ter_up_df.to_csv('fillallmissing.csv', index=None)
    return ter_up_df

def combinecsv(pathes):
    filepathes = pathes + '*_newhaddletrain.csv'
    # print(filepathes)
    df_list = []
    for path in glob.glob(filepathes):
        print(path)
        df_list.append(pd.read_csv(path))
    df = pd.concat(df_list).reset_index(drop=True)
    df.to_csv(pathes+'allnewhaddledata.csv', index=True)
    return df

if __name__ == '__main__':
    df = pd.read_csv('/Users/honey/Desktop/tianjinbus/newtraindata/allnewhaddledata.csv')
    haddle_missing(df)
